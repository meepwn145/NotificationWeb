...
const Reservation = () => {
    ...
    const fetchReservations = async (managementName) => {
        ...
        try {
          ...
          const tokenDoc = await getDoc(doc(db, "userTokens", userEmail));
  ...
  const fetchTokenByUser = async (email) => {
    try {
      // Fetch the user's document from the 'userTokens' collection using email as the document ID
      const tokenDocRef = doc(db, 'userTokens', email);
      const tokenDoc = await getDoc(tokenDocRef);
      
      if (tokenDoc.exists()) {
        const tokenData = tokenDoc.data();
        console.log('Token', tokenData.token);
        return tokenData.token; // Return the token from Firestore
      } else {
...
const handleReservation = async (accepted, reservationRequest, index) => {
  const { id, userName, carPlateNumber, slotId, timeOfRequest, floorTitle, userEmail, accessToken} = reservationRequest;
  const status = accepted ? "Accepted" : "Declined";

  // Log entry for the history
  const logEntry = {
    status,
    name: userName,
    carPlateNumber,
  ...
  if (accepted) {
    try {
      ...
      // Fetch the user's token from Firestore using their email
      const userToken = await fetchTokenByUser(userEmail);

      if (userToken) {
        const notificationData = {
          appId: 24190,  // your appId from Native Notify
          appToken: '7xmUkgEHBQtdSvSHDbZ9zd', // your appToken from Native Notify
          title: 'Reservation',
          message: `Your reservation at ${user.managementName} on ${floorTitle} ${slotId + 1} is accepted`,
          targetUsers: [accessToken], // Target the user's token
          subID: userEmail,  // Token identifier for the logged-in user
          color: '#FF0000FF',
        };
        ...
        alert("No valid token to send notification.");
      }

      alert(`Reservation accepted for ${userName} at slot ${slotId + 1}.`);

      // Store reservation status in resStatus collection
